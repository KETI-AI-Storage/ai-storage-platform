#!/usr/bin/env bash
# 02-Provisioning-apply-result.sh는 Provisioning scope 정책의 실제 kubectl CR 결과를 출력한다.
#
...
start="$(date +%s)"
print_header "Provisioning Apply Result"
...
if kubectl api-resources --no-headers 2>/dev/null | awk '{print $1}' | grep -q '^orchestrationpolicies$'; then
  print_commands_block "command" \
    ...
    "kubectl get orchestrationpolicy -A -o wide | grep -i provisioning"
  raw="$(capture_cmd 'kubectl get orchestrationpolicy -A -o wide')"
  filtered="$(printf '%s\n' "${raw}" | grep -iE 'NAME|provisioning' || true)"
  print_raw_block "orchestrationpolicy" "${filtered}"
  if printf '%s' "${raw}" | grep -qi 'provisioning'; then
