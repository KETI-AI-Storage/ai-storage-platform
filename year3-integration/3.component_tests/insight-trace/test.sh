...
NAMESPACE="apollo"
DEPLOYMENT_NAME="insight-trace"
SERVICE_NAME="insight-trace"
SERVICE_PORT="8080"

print_header "Insight Trace Component Test"
...
if ! kubectl get deployment $DEPLOYMENT_NAME -n $NAMESPACE &>/dev/null; then
    ...
    log_info "Please deploy insight-trace first"
    exit 1
fi
...
create_curl_pod "test-insight-trace" "default"

# Test health endpoint
log_test "Testing health endpoint..."
HEALTH_RESPONSE=$(exec_curl "test-insight-trace" "default" "http://${SERVICE_IP}:${SERVICE_PORT}/health" "GET" 2>/dev/null || echo "")
...
# Test trace endpoint (if exists)
log_test "Testing trace API..."
TRACE_RESPONSE=$(exec_curl "test-insight-trace" "default" "http://${SERVICE_IP}:${SERVICE_PORT}/api/traces" "GET" 2>/dev/null || echo "")
if [ -n "$TRACE_RESPONSE" ]; then
    log_info "Trace API available"
    run_test "Trace API" "true"
else
    log_info "Trace API not available or empty response"
fi

# ============================================
# Tracing Test
# ============================================
log_step "5. Tracing Functionality Test"

# Send a test trace
log_test "Sending test trace..."
TEST_TRACE='{"serviceName":"test-service","operationName":"test-operation","duration":100}'
TRACE_RESULT=$(exec_curl "test-insight-trace" "default" "http://${SERVICE_IP}:${SERVICE_PORT}/api/v1/trace" "POST" "$TEST_TRACE" 2>/dev/null || echo "")
if [ -n "$TRACE_RESULT" ]; then
    log_info "Trace submission response: $TRACE_RESULT"
fi
...
cleanup_curl_pod "test-insight-trace" "default"
