...
PARAMS
      # WHY: set -e 환경에서 [[ -n "" ]] && echo 는 exit 1 이라 command substitution
      #      $(build_policy_parameters) 가 즉시 실패한다. if 로 분리한다.
      if [[ -n "${mig_pod}" ]]; then
        echo "    podName: \"${mig_pod}\""
      fi
      if [[ -n "${DEMO_PARAM_TARGET_NODE:-}" ]]; then
        echo "    targetNode: \"${DEMO_PARAM_TARGET_NODE}\""
      fi
      ;;
...
# WHY: migration 은 parameters.podName 으로 실제 소스 Pod 를 지정한다.
#      scenario1 의 WL_NODE(gpu-server-03) 를 spec.targetNode 에 넣으면
#      Apollo 가 없는 노드로 이주를 시도해 즉시 Failed 가 된다.
#      parameters.targetNode 가 있으면 그 값을 spec 에도 반영하고,
#      없으면 spec.targetNode 를 비워 pickAlternateNode 가 동작하게 한다.
POLICY_SPEC_TARGET_NODE=""
case "${POLICY_TYPE_RESOLVED}" in
  migration)
    POLICY_SPEC_TARGET_NODE="${DEMO_PARAM_TARGET_NODE:-}"
    ;;
  *)
    POLICY_SPEC_TARGET_NODE="${WL_NODE}"
    ;;
esac
...
$(if [[ -n "${POLICY_SPEC_TARGET_NODE}" ]]; then echo "  targetNode: ${POLICY_SPEC_TARGET_NODE}"; fi)
